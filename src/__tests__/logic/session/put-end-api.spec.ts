...
describe('PutEndApiのテスト', () => {
  ...
  test('200が返ってきたらtrueを返すこと', async () => {
    ...
    const result = await new PutEndApi(inputForm).put()
  ...
  test('400が返ってきたら例外を投げること', async () => {
    const mockSend = vi
      .spyOn(PutApi.prototype, 'send')
      .mockResolvedValue(new ApiResponse(400, false, { message: 'Invalid Param' }))

    const api = new PutEndApi(inputForm)

    await expect(api.put()).rejects.toThrow('API error: Invalid Param')
    expect(mockSend).toHaveBeenCalledTimes(1)
